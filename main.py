...
#%%
# Load datasets
ICGC = load_data("/Users/uw20204/CanDrivR_publishing/CanDrivR-CS/data/ICGC.tsv.gz")
COSMIC = load_data("/Users/uw20204/CanDrivR_publishing/CanDrivR-CS/data/COSMIC.tsv.gz")
#%%
# Check to ensure that no ICGC training variants are found in the COSMIC test data.
check_duplicates_between_datasets(ICGC, COSMIC)
#%%
# Train and evaluate baseline pan-cancer model.
final_model, feature_importance = train_baseline_model(ICGC, features, save_path_test = f"{OUTPUT_DIR}/baseline_test_results.tsv", save_path_cross_val = f"{OUTPUT_DIR}/baseline_cross_val_results.tsv")
#%%
# Evaluate baseline model on COSMIC data.
baseline_accuracy = evaluate_model_on_cosmic(final_model, COSMIC, features, save_path = f"{OUTPUT_DIR}/COSMIC_eval_results.tsv")

#%%
# Split ICGC training data into cancer types.
...
#%%
# Plot optimised donor counts for each cancer dataset
plot_grid_of_curves(cancer_specific_list, f"{OUTPUT_DIR}/", cancer_specific_df["cancer"].unique())

#%%

# Get cross-validation and test results for heatmap plotting
optimised_metrics_cross_val, optimised_metrics_test = get_test_cross_val_res(optimised_cancer_results, cancer_specific_df, cancer_dict_donor_counts, 
                                                                             cross_val_path = f"{OUTPUT_DIR}/cancer_specific_cross_val_results.tsv", 
                                                                             test_path = f"{OUTPUT_DIR}/cancer_specific_test_results.tsv")

# %%
# Plot heatmaps for test and validation metrics
...
# Extract feature importance for each cancer type and plot heatmap
standardised_heatmap_data = plot_feature_importance_heatmap(ICGC, optimised_metrics_cross_val, cancer_dict_donor_counts, OUTPUT_DIR, features)
# %%
ICGC = load_data("/Users/uw20204/CanDrivR_publishing/CanDrivR-CS/data/ICGC.tsv.gz")

# Run model on TCGA UCEC dataset
...
run_and_test_tcga_model(ICGC = ICGC, results_file = f"{OUTPUT_DIR}/TCGA_UCEC_results.tsv", TCGA_df = UCEC_updated, study_id='UCEC', positive_dataset_donor_count=3, features=features)
#%%
ICGC
# %%
# Run model on TCGA UCEC dataset
...
run_and_test_tcga_model(ICGC = ICGC, results_file = f"{OUTPUT_DIR}/TCGA_SKCM_results.tsv", TCGA_df = TCGA_SKCM, study_id='SKCM', positive_dataset_donor_count=4, features=features)
...
DATA_DIR = "/Users/uw20204/CanDrivR_publishing/CanDrivR-CS/data"
OUTPUT_DIR = "/Users/uw20204/CanDrivR_publishing/CanDrivR-CS/output"
